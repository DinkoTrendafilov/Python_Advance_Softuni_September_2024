...
for day in range(0, number_of_days + 1):
    ...
    print(f"Day: {day} - Combinations of day to all days: {(math.comb(number_of_days, day)):_}")
...
for (key, value) in sorted_dictionary.items():
    ...
    print(f"Rainy days: {key:_} || Times: {value} || {(value * 100):.4f} %")

print()
print(f"Sum of top 5 values is: {(sum(res) * 100):.2f} %")
print()
for (num, value) in top_5_results:
    print(f"Rainy days: {num} || Times: {value:.4f} || {(value * 100):.2f} %")

print()
print(f"Total combinations: {(6 ** number_of_days):_}")
